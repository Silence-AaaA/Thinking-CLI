...
async function testObservationManager() {
  ...
  const summaryObs = manager.observe(makeToolCall("file_summary", { path: "src/main.ts" }), {
    success: true,
  ...
  const grepObs = manager.observe(makeToolCall("grep", { pattern: "TODO" }), {
    success: true,
    data: {
      pattern: "TODO",
      matchesFound: 8,
  ...
  const shellObs = manager.observe(makeToolCall("run_shell", { command: "npm test" }), {
    success: true,
    data: {
      command: "npm test",
      exitCode: 0,
  ...
  const failObs = manager.observe(makeToolCall("run_shell", { command: "npm test" }), {
    success: false,
    data: {
      command: "npm test",
      exitCode: 1,
  ...
  const enoentObs = manager.observe(makeToolCall("read_file", { path: "/nonexistent/file.txt" }), {
    success: false,
  ...
  const gitObs = manager.observe(makeToolCall("git_status", {}), {
    success: true,
    data: {
      branch: "main",
      staged: ["src/app.ts"],
